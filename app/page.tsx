...
export default function Home(){
 ...
 const {data,loading,error,refresh}=useQuotes(amount);const routes=data?.routes?.length?data.routes:fallback;const current=routes.find(r=>r.id===selected)||routes[0];
 ...
 return <main><nav><div className="brand"><i>↗</i>Crosschain <b>Router</b></div><div className="navlinks"><a href="#swap">Swap</a><a href="#routes">Routes</a></div><button className="wallet"><Wallet size={16}/> Connect wallet</button></nav>
 ...
 <button className="primary" disabled={expired}>{expired?"Refresh quote to continue":<><Wallet size={17}/> Connect wallet to continue</>}</button><p className="safe"><ShieldCheck size={13}/> Non-custodial · You approve every transaction</p></div>
