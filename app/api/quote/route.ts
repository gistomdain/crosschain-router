...
export async function POST(request:Request){
 ...
 const userAddress=body.userAddress===undefined?undefined:String(body.userAddress);if(userAddress!==undefined&&!/^0x[a-fA-F0-9]{40}$/.test(userAddress))return NextResponse.json({error:"Invalid EVM wallet address"},{status:400});
 const input={fromChain:body.fromChain??1,toChain:body.toChain??"solana",fromToken:body.fromToken??"USDC",toToken:body.toToken??"USDC",amount:amountText,userAddress};const sameChain=isSameChain(input);
 ...
 const routes=live.flatMap(q=>{const receive=Number(q.receive);if(!Number.isFinite(receive)||receive<=0)return[];return[{id:q.id,provider:q.provider,receive,fee:q.feeUsd,eta:q.etaSeconds?formatEta(q.etaSeconds):"—",steps:q.steps,expiresAt:q.expiresAt??expiresAt,approvalTxs:q.approvalTxs??[],tx:q.tx,tracking:q.tracking}]});if(routes.length)return NextResponse.json({mode:"live",routeType:sameChain?"swap":"cross-chain",requestedAt:new Date().toISOString(),providerHealth:aggregate.health,routes});
