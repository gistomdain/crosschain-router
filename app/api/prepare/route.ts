...
export async function POST(request:Request){
 ...
 try{
  ...
  if(!/^\\d+(?:\\.\\d+)?$/.test(reviewedText))return NextResponse.json({error:"Invalid reviewed route amount"},{status:400});let reviewedUnits:bigint;try{reviewedUnits=fixed(reviewedText)}catch{return NextResponse.json({error:"Unsupported reviewed route precision"},{status:400})}if(reviewedUnits<=0n)return NextResponse.json({error:"Invalid reviewed route amount"},{status:400});let deteriorationBps=0;if(freshUnits<reviewedUnits)deteriorationBps=Number((reviewedUnits-freshUnits)*10000n/reviewedUnits);const fresh=receiveText;
