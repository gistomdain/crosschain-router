...
export async function POST(request:Request){
 ...
 try{
  ...
  const receiveText=String(quote.receive);if(receiveText.length===0||receiveText.length>120||!/^\\d+(?:\\.\\d+)?$/.test(receiveText))return NextResponse.json({error:"Provider returned an invalid destination amount"},{status:502});
  const reviewedText=String(body.reviewedReceive??"");const SCALE=10n**18n;const fixed=(value:string)=>{const [whole="0",fraction=""]=value.split(".");if(fraction.length>18)throw new Error("Destination amount precision exceeds supported comparison precision");return BigInt(whole)*SCALE+BigInt((fraction+"0".repeat(18)).slice(0,18))};let freshUnits:bigint;try{freshUnits=fixed(receiveText)}catch{return NextResponse.json({error:"Provider returned an invalid destination amount"},{status:502})}if(freshUnits<=0n)return NextResponse.json({error:"Provider returned an invalid destination amount"},{status:502});
  if(!/^\\d+(?:\\.\\d+)?$/.test(reviewedText))return NextResponse.json({error:"Invalid reviewed route amount"},{status:400});let reviewedUnits:bigint;try{reviewedUnits=fixed(reviewedText)}catch{return NextResponse.json({error:"Unsupported reviewed route precision"},{status:400})}if(reviewedUnits<=0n)return NextResponse.json({error:"Invalid reviewed route amount"},{status:400});let deteriorationBps=0;if(freshUnits<reviewedUnits)deteriorationBps=Number(((reviewedUnits-freshUnits)*10000n+reviewedUnits-1n)/reviewedUnits);const fresh=receiveText;
  ...
  return NextResponse.json({provider:quote.provider,receive:fresh,fee,etaSeconds,expiresAt,approvalTxs:approvals,tx:quote.tx,tracking,capabilities:quote.capabilities,routeMeta,deteriorationBps,requiresReconfirm:false})
