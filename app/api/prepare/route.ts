...
export async function POST(request:Request){
 ...
 try{
  ...
  let deteriorationBps=0;if(/^\\d+(?:\\.\\d+)?$/.test(reviewedText)){try{const reviewedUnits=fixed(reviewedText);if(reviewedUnits>0n&&freshUnits<reviewedUnits)deteriorationBps=Number((reviewedUnits-freshUnits)*10000n/reviewedUnits)}catch{}}const fresh=receiveText;
