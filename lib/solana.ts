...
export function parseSolanaAmount(amount:string,decimals:number){if(!/^\d+(?:\.\d+)?$/.test(amount)||decimals<0||!Number.isInteger(decimals))return null;const [whole="0",fraction=""]=amount.split(".");if(fraction.length>decimals)return null;try{const value=toSolanaBaseUnits(amount,decimals);return BigInt(value)>0n?value:null}catch{return null}}
