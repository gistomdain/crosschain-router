...
export type RouteChange={kind:"better-route"|"selected-improved"|"selected-worsened";previous:QuoteSnapshot;next:QuoteSnapshot;delta:number;deltaText:string;deltaBps:number};
...
export function detectRouteChange(previous:QuoteSnapshot[],next:QuoteSnapshot[],selectedId:string,minImprovementBps=1):RouteChange|null{
 ...
 if(best&&best.id!==selectedId){
  ...
  if(deltaBps>=minImprovementBps)return{kind:"better-route",previous:newSelected,next:best,delta,deltaText:differenceText(best.receive,newSelected.receive),deltaBps};
 }
 ...
 const direction=compareReceive(newSelected.receive,oldSelected.receive);if(deltaBps>=minImprovementBps&&direction!==0)return{kind:direction>0?"selected-improved":"selected-worsened",previous:oldSelected,next:newSelected,delta,deltaText:differenceText(newSelected.receive,oldSelected.receive),deltaBps};
 return null;
}