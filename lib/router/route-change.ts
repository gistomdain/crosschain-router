...
const scaled=(value:number|string)=>{const text=String(value);if(!/^\\d+(?:\\.\\d+)?$/.test(text))return null;const [whole,fraction=""]=text.split(".");try{return BigInt(whole)*SCALE+BigInt((fraction+"00000000").slice(0,8))}catch{return null}};
...
export function detectRouteChange(previous:QuoteSnapshot[],next:QuoteSnapshot[],selectedId:string,minImprovementBps=1):RouteChange|null{
 ...
 const oldReceive=receiveNumber(oldSelected.receive),newReceive=receiveNumber(newSelected.receive);const oldExact=scaled(oldSelected.receive),newExact=scaled(newSelected.receive);const delta=newReceive-oldReceive;const deltaBps=oldExact&&oldExact>0n&&newExact!==null?Number((newExact>=oldExact?newExact-oldExact:oldExact-newExact)*10000n/oldExact):Math.abs(bps(delta,oldReceive));
